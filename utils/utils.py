...
def Cluster(X_train,k,D,t,beta):
  ...
  if len(cluster_no) > 1:
    ...
    # for i in index:
    #   a = torch.where(cluster_no == -1)
    #   a = a[0][0]
    #   t = torch.cat((cluster_no[0:a],cluster_no[a+1:len(cluster_no)]),axis=0)
    #   aiGuess=random.choice(t)
    #   CC[i] = aiGuess
...
def synthetic_generation(CC,X_train,Y_train,t):
  ...
  #Displaying after assigning clusters
  ...
  for c in No:
    ...
    print("maj", majority_class, "min", minority_class)
...
def test(ep,model, test_loader, device=None):
  ...
  #temp1 = balanced_accuracy_score(groundtruth_labels_train.tolist(),prediction_labels_train.tolist())
  #print("train bacc:", temp1)
  # if balanced_train<temp:
  #   balanced_train = temp
  #   epoch_train = ep
  
  balanced_test = balanced_accuracy_score(groundtruth_labels_test.tolist(),prediction_labels_test.tolist())
  #print("test bacc:", temp)
  # if balanced_test<temp:
  #   balanced_test = temp
  #   #epoch_test = ep
  #   #epoch_train = ep
  #   #balanced_train = temp1


  gt = groundtruth_labels_test.cpu().numpy().astype(int)
  p = prediction_labels_test.cpu().numpy().astype(int)
  mcc = sklearn.metrics.matthews_corrcoef(gt, p)#, *, sample_weight=None)
  # print(gt.dtype)
  # print(p.dtype)
  # print(p)
  # print(gt)
  # print(type(p))
  f1_scores = sklearn.metrics.f1_score(gt,p, average='weighted')#groundtruth_labels_train.cpu().numpy().tolist(), prediction_labels_train.cpu().numpy().tolist(), average='weighted')
  gmean = imblearn.metrics.geometric_mean_score(gt.tolist(),p.tolist())#groundtruth_labels_train.cpu().numpy().tolist(), prediction_labels_train.cpu().numpy().tolist(), labels=None, pos_label=1, average='multiclass', sample_weight=None, correction=0.0)
  # if mcc<temp_mcc:
  #   mcc = temp_mcc


  # if f1_scores < temp_f1_score:
